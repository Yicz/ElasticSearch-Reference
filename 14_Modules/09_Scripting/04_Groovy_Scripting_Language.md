...
Returns Euler’s number _e_ raised to the power of value.  
  
`log(a)`

| 

Returns the natural logarithm (base _e_ ) of a value.  
...
Returns the angle _theta_ from the conversion of rectangular coordinates ( _x_ , _y_ ) to polar coordinates (r, _theta_ ).  
...
Returns the closest _int_ to the argument.  
  
`random()`

| 

Returns a random _double_ value.  
...
Returns sqrt( _x2_ \+ _y2_ ) without intermediate overflow or underflow.
